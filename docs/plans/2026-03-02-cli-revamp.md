...
- Missing to claim full completion:
- `cli/src/cmd/cp.rs` still exists and should be removed for full phase closure.
- `--stdin=pipe` currently spools to file but does not implement per-task streaming cursor semantics + cleanup policy.
- Integration tests listed in Phase 9 are not yet implemented.
- `cargo clippy --workspace --all-targets -- -D warnings` currently fails due existing strict lint violations (mostly in `lib/src/db/duckdb.rs` and existing resolver signatures).
- `cargo test --workspace` not green (existing resolver tests fail under current environment).
- Ctrl-C second-interrupt force-kill semantics are not implemented to book spec.
